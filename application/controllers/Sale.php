    ...
    function add_sale_by_ajax(){
        ...
        if($sale_id>0){
            ...
            $create_payload = array(
                'sale' => $data,
                'orders_table' => isset($order_details->orders_table) ? $order_details->orders_table : array(),
                'items' => isset($order_details->items) ? $order_details->items : array()
            );
            $create_result = $this->Sale_write_model->create_running_order($create_context, $create_payload);
            if($create_result['success']){
                $sales_id = $create_result['sale_id'];
                echo $sales_id;
            }
        }
        if($close_order==1){
